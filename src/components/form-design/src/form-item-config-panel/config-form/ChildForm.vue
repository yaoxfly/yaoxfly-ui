...
import { chartConfigFormTreeBuilder } from './parse-utils/chart-config-form-tree-builder'
import { buildReder, buildChartForm } from './parse-utils/build-chart-form-config-render'
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      // console.log('name ', this.root, ' ', this.formTree, ' ', this.form)
    ...
    getFormFieldData (formPropertyName) {
      const curChartOptions = this.activateItem
      /**
       * 从chart配置对象中读取某个字段的值
       */
      return getPropertyDataFromNestObj(curChartOptions, `${this.formPropertyName}.${formPropertyName}`)
    },
    /**
     * 更新表单
     */
    commitUpdate (formPropertyName, value, oldValue, canRedo) {
      this.updateFormItemConfig({
        fieldName: `${this.formPropertyName}.${formPropertyName}`,
        value,
        oldValue,
